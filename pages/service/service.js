...
Page({
  ...
  closeGuid: function () {
    var that = this;
    that.setData({animationData0: {}, userGuid: 0})
  },
  ...
  change_service: function (e) {
    var that = this;
    this.setData({service: e.target.dataset.index}) //animationData_progress  translateX(107*e.target.dataset.index)
  ...
  orderSubmit: function () {
    if (this.data.service == 1 && this.data.proconfirm == 0) {
      ...
      if (!filePath || !addressid || !date || !time || !msg) {
        wx.showToast({title: '请认真填写~', image: '../../static/image/tip.png', duration: 2000})
      } else {
        ...
        wx.uploadFile({
          ...
          success: function (res) {
            if (res.data) 
              setTimeout(function () {
                wx.hideLoading()
                wx.showToast({title: '上传成功', duration: 1000, icon: "success"})
                that.setData({
